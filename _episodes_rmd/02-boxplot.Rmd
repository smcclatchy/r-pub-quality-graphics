...
The median red blood cell count across all strains is `r median(cc_data$RBC, na.rm=TRUE)` and the mean value is `r mean(cc_data$RBC, na.rm=TRUE)`. You can view the distribution of red blood cell values with a histogram.

```{r hist, include=FALSE}
ggplot(data = cc_data, mapping = aes(x = RBC)) + geom_histogram()
```

The histogram shows that the data are centered around the value of `r median(cc_data$RBC, na.rm=TRUE)`.  
